...
public class BlockPathFinder
{
    private static boolean isValidBlock(final LineBlock check, final Block end, final Set<LineBlock> banned) {
        ...
        final WrappedMaterial material = WrappedMaterial.get(check.getBlock(end.getWorld()).getType());
    ...
    private static Set<LineBlock> line(final Block start, final Block end) {
        final Set<LineBlock> confirmed = (Set<LineBlock>)Sets.newHashSet();
        final Set<LineBlock> banned = (Set<LineBlock>)Sets.newHashSet();
    ...
    public static Set<Block> line(final Location start, final Location end) {
        return toBlocks(start.getWorld(), line(start.getBlock(), end.getBlock()));
    }
    
    private static Set<Block> toBlocks(final World world, final Set<LineBlock> lineBlocks) {
        final Set<Block> blocks = (Set<Block>)Sets.newHashSet();
